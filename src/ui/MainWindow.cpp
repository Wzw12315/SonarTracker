...
private:
    ...
    void loadJson() {
        ...
        if (rootObj.contains("targets") && rootObj["targets"].isArray()) {
            ...
            for (int i = 0; i < targetArray.size(); ++i) {
                ...
                truth.name = tObj["name"].toString(); truth.initialAngle = tObj["initialAngle"].toDouble();
                truth.initialDistance = tObj["initialDistance"].toDouble(); truth.speed = tObj["speed"].toDouble();
                truth.course = tObj["course"].toDouble(); truth.trueDemonFreq = tObj["trueDemonFreq"].toDouble();
    ...
    void saveJson() {
        ...
        for (const auto& t : getTruthData()) {
            QJsonObject tObj; tObj["id"] = t.id; tObj["name"] = t.name; tObj["initialAngle"] = t.initialAngle;
            tObj["initialDistance"] = t.initialDistance; tObj["speed"] = t.speed; tObj["course"] = t.course;
            tObj["trueDemonFreq"] = t.trueDemonFreq; QJsonArray lofarArr;
            for (double f : t.trueLofarFreqs) lofarArr.append(f);
            tObj["trueLofarFreqs"] = lofarArr; targetArray.append(tObj);
        }
...
void MainWindow::onStartClicked() {
    ...
    if (truths.empty()) {
        ...
        m_lblModeIndicator->setText(QString("🟢 先验知识模式 (真值:%1个)").arg(truths.size()));
        ...
        m_logConsole->appendPlainText(QString("[系统提示] 已加载 %1 个先验真值目标，系统进入【先验知识模式】。\n").arg(truths.size()));
    }
...
void MainWindow::setupUi() {
    ...
    QGroupBox* gDcv = new QGroupBox("反卷积 (DCV) 设置", paramContainer);
    ...
    m_spatialPlot->addGraph(); m_spatialPlot->graph(1)->setName("DCV (反卷积)"); m_spatialPlot->graph(1)->setPen(QPen(Qt::blue, 2));
    ...
    m_dcvWaterfallPlot->plotLayout()->insertRow(0); m_dcvWaterfallPlot->plotLayout()->addElement(0, 0, new QCPTextElement(m_dcvWaterfallPlot, "反卷积波束形成(DCV) 空间谱历程", QFont("sans", 12, QFont::Bold)));
...
void MainWindow::onEvaluationResultReady(const SystemEvaluationResult& res) {
    ...
    // 【意见三修改：在填充表格前，先断开信号，防止 setText 触发无限死循环】
        ...
        for (int i = 0; i < res.targetEvals.size(); ++i) {
            ...
            nameItem->setData(Qt::UserRole, eval.targetId); // 绑定真实ID
            nameItem->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEditable | Qt::ItemIsEnabled); // ★ 允许双击编辑
        ...
        // 【意见三修改：表格数据全部填充完毕后，重新连接信号，监听用户的实时改名操作】
        connect(m_tableTargetFeatures, &QTableWidget::itemChanged, this, &MainWindow::onTargetNameChanged);
    // ==========================================================
    // 【意见二修改】：动态遍历真值图和解算图，获取上下界限并自适应 Y 轴缩放
    // ==========================================================
    ...
    if (hasTrueRange) {
        m_plotTrueAzimuth->yAxis->setRange(trueMin - 5.0, trueMax + 5.0);
    } else {
        m_plotTrueAzimuth->yAxis->setRange(0, 180);
    }
    ...
    if (hasCalcRange) {
        m_plotCalcAzimuth->yAxis->setRange(calcMin - 5.0, calcMax + 5.0);
    } else {
        m_plotCalcAzimuth->yAxis->setRange(0, 180);
    }
